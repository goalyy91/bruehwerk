...
export type Route =
  ...
  | { name: 'verkostungBearbeiten'; shotId: string }
  | { name: 'getraenke' }
...
export function zuPfad(route: Route): string {
  switch (route.name) {
    ...
    case 'verkostungBearbeiten':
      return `/historie/${route.shotId}/verkostung/bearbeiten`;
    case 'getraenke':
      return '/getraenke';
    case 'getraenk':
    ...
    case 'verhalten':
      return '/einstellungen/verhalten';
    case 'musterblatt':
    ...
    case 'sicherungen':
      return '/einstellungen/sicherungen';
  }
}
...
export function ausPfad(pfad: string): Route {
  ...
  if (t[0] === 'historie') {
    ...
    if (t.length === 4 && t[2] === 'verkostung' && t[3] === 'bearbeiten') return { name: 'verkostungBearbeiten', shotId: t[1]! };
  }
  ...
  if (t[0] === 'einstellungen') {
    ...
    if (t.length === 2 && t[1] === 'sicherungen') return { name: 'sicherungen' };
...
export function elternVon(route: Route): Route | undefined {
  switch (route.name) {
    ...
    case 'verkostungBearbeiten':
      return { name: 'verkostung', shotId: route.shotId };
    case 'kaffeeNeu':
      return { name: 'kaffees' };
    case 'kaffee':
      return { name: 'kaffees' };
